...
if (!empty($filters)) {
    foreach ($filters as $key => $value) {
        ...
        if ($key === 'price_min' || $key === 'price_max') {
            // Ensure the key exists in the filters and handle numeric values
            if ($key === 'price_min' && is_numeric($value)) {
                $sql .= " AND price >= '$value'";
            } elseif ($key === 'price_max' && is_numeric($value)) {
                $sql .= " AND price <= '$value'";
            }
        } else {
            $sql .= " AND $key = '$value'";
